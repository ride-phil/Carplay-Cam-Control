...
- **Cross-widget refresh reliability**: confirmed bug — stopping a camera via one widget left `RecordAllWidget`'s count stuck (e.g. "1/3 recording") until directly interacting with `RecordAllWidget` itself forced a recompute; starting had worked fine moments earlier. Root cause: a widget's *own* button press gets an immediate, non-throttled refresh, but `reloadAllTimelines()` telling *other* widgets to refresh goes through iOS's normal rate-limited background refresh budget — not instant, and can sit unrefreshed indefinitely. Two-layer fix, since "eventually maybe" isn't acceptable for this app's whole purpose: (1) `SharedState` setters now call `UserDefaults.synchronize()` to force an immediate cross-process flush, in case of any `cfprefsd` visibility delay on top of the throttling; (2) both widgets' timeline policies changed from `.never` to `.after(20s)`, so they self-correct on their own cadence regardless of whether a pushed reload landed, instead of relying entirely on `reloadAllTimelines()` being honored promptly. Not yet re-tested on hardware. The 20s interval is a starting guess, not measured — may need tuning against the real-world WidgetKit daily refresh budget (tighter for faster correction vs. risk of exhausting the budget if it's used heavily on a long ride).
...
- [ ] Re-test cross-widget refresh (start/stop via one widget, check the other) on hardware after the `UserDefaults.synchronize()` + 20s periodic-refresh fix — confirm the fix actually closes the gap, and tune the 20s interval against real-world WidgetKit refresh budget behavior (faster correction vs. budget exhaustion on long rides)
